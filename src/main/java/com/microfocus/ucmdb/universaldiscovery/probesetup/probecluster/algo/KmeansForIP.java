...
public class KmeansForIP
{
    ...
    public KmeansForIP(String fileName, String labelname)
    {
        ...
        try
        {
            ...
            reader = new BufferedReader(new FileReader(fileName));
            int nrow=0;
            while ((values = CSVHelper.parseLine(reader))!=null){
    ...
    public KmeansForIP(double[][] _data, int _nrows, int _ndims) {
        this._data = _data;
        this._nrows = _nrows;
        this._ndims = _ndims;
    }
    ...
    private int closest(double [] v){
        double mindist = euclidenDist(v, _centroids[0]);
        int label =0;
        for (int i=1; i<_numClusters; i++){
            double t = euclidenDist(v, _centroids[i]);
            if (mindist>t){
                mindist = t;
                label = i;
            }
    ...
    private double euclidenDist(double [] v1, double [] v2){
        double sum=0;
...
//    private double binaryDist(double [] v1, double [] v2){
//        double sum = 0;
//        for(int i = _ndims - 1; i >= 0; i--){
//            double d = Math.abs((v1[i] - v2[i]) * Math.pow(2, _ndims - 1 - i));
//            sum += d;
//        }
//        return sum;
//    }
    ...
    private boolean converge(double [][] c1, double [][] c2, double threshold){
        ...
        double maxv = 0;
        for (int i=0; i< _numClusters; i++){
            double d= euclidenDist(c1[i], c2[i]);
            if (maxv<d){
                maxv = d;
            }
        }
    ...
    public static void main( String[] astrArgs )
    {
         ...
         *
         */
        KmeansForIP KM = new KmeansForIP( "data.csv", null );
